...
// Activity: computed from goLiveDate relative to today
// "Active"   = went live within the last 10 days
// "Exhausted"= went live more than 10 days ago
// "Upcoming" = not yet live (status === "Scheduled")
type Activity = "Active" | "Exhausted" | "Upcoming";

const TODAY = "2026-06-22"; // dashboard reference date (update daily)
...
function exportCSV(rows: VideoRow[]) {
  const headers = [
    "Title", "Creator", "Platform", "Format", "Live Date",
    "Views", "Clicks", "Installs", "CPI", "Click→Install%",
    "Net Cost", "Status", "URL",
  ];
...
export default function VideosPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return allRows.filter((r) => {
      if (q && !r.title.toLowerCase().includes(q) && !r.creatorName.toLowerCase().includes(q))
        return false;
  ...
  const columns: Column<VideoRow>[] = [
    ...
    },
    {
      ...
      getValue: (r) => r.format,
      render: (r) => <Badge label={r.format} className={formatBadge[r.format] ?? ""} />,
    },
    {
    ...
    {
      key: "clickThroughs", label: "Clicks", sortable: true,
    ...
    },
    {
      ...
      getValue: (r) => r.activity,
      render: (r) => <Badge label={r.activity} className={ACTIVITY_BADGE[r.activity]} />,
    },
    {
      key: "link", label: "",
      render: (r) => (
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--text-secondary)" }}>
            Installs attributed via Dub · Click headers to sort
          </p>
        </div>
        <button
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {/* Platform pills */}
        {PLATFORMS.map((p) => (
          <PillButton key={p} label={p} active={platform === p} onClick={() => setPlatform(p)} />
        ))}

        <div className="w-px h-5 mx-1" style={{ background: "var(--border)" }} />

        {/* Status pills — Live / Scheduled */}
        {STATUSES.map((s) => (
        ...
        {/* Activity pills — Active / Exhausted / Upcoming */}
        {ACTIVITIES.map((a) => (
          <PillButton key={a} label={a} active={activity === a} onClick={() => setActivity(a)} />
        ))}
