...
export default function PerformancePage() {
  ...
  const allRows: PerfRow[] = useMemo(() => {
    return performances.map((p) => {
      ...
      const clickToInstallRate = p.clickThroughs > 0 ? (videoInstalls / p.clickThroughs) * 100 : 0;
  ...
  const columns: Column<PerfRow>[] = [
    ...
    },
    {
      ...
      getValue: (r) => r.views,
      render: (r) => <span className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>{formatNumber(r.views)}</span>,
    },
    {
    ...
    },
    {
      ...
      render: (r) => <span className="text-sm" style={{ color: "var(--text-secondary)" }}>{formatNumber(Math.round(r.watchTimeMinutes / 60))}h</span>,
    },
    {
      key: "clickThroughs", label: "Dub Clicks", sortable: true,
      getValue: (r) => r.clickThroughs,
      render: (r) => <span className="text-sm" style={{ color: "var(--text-secondary)" }}>{formatNumber(r.clickThroughs)}</span>,
    },
    {
    ...
    },
    {
      ...
      render: (r) => r.revenue ? (
        <span className="text-sm font-medium text-emerald-500">{formatCurrency(r.revenue)}</span>
      ) : <span style={{ color: "var(--text-muted)" }}>—</span>,
    },
    {
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        <StatCard label="Total Views" value={formatNumber(totalViews)} icon={Eye} iconColor="text-indigo-500" iconBg="bg-indigo-100 dark:bg-indigo-500/10" />
        <StatCard label="Total Installs" value={formatNumber(totalInstalls)} sub="via Dub attribution" icon={Download} iconColor="text-rose-500" iconBg="bg-rose-100 dark:bg-rose-500/10" />
        ...
        <StatCard label="Total Watch Time" value={`${formatNumber(totalWatchHrs)}h`} icon={Clock} iconColor="text-violet-500" iconBg="bg-violet-100 dark:bg-violet-500/10" />
      </div>
