...
import { creators, getAllCreatorMetrics } from "@/lib/mock-data";
...
export default function CreatorsPage() {
  const allMetrics = getAllCreatorMetrics();
  ...
  const columns: Column<CreatorRow>[] = [
    ...
    },
    {
      ...
      render: (r) => <span className="text-sm" style={{ color: "var(--text-secondary)" }}>{formatNumber(r.followers)}</span>,
    },
    {
      key: "totalSpend", label: "Spend", sortable: true,
      getValue: (r) => r.totalSpend,
      render: (r) => <span className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>{r.totalSpend ? formatCurrency(r.totalSpend) : "—"}</span>,
    },
    {
      key: "totalInstalls", label: "Installs", sortable: true,
      getValue: (r) => r.totalInstalls,
      render: (r) => <span className="text-sm font-semibold text-indigo-500">{r.totalInstalls ? formatNumber(r.totalInstalls) : "—"}</span>,
    },
    {
